...
class SurfService {
  ...
  async warmupProfile() {
    ...
    // --- STEP: Honeygain Lucky Pot ---
    try {
      console.log("Opening Honeygain dashboard...");

      await this.driver.get("https://dashboard.honeygain.com/");
      await sleep(3000);
      ...
      await this.driver.get("https://www.youtube.com/watch?v=rgfBqIGoQUg");
      await sleep(20000);
      ...
      try {
        ...
        for (let i = 0; i < ads.length; i++) {
          ...
          try {
            ...
            if (attemptedAds.has(adId)) continue;
            if (blockedAds.includes(adId)) continue;
        ...
        for (const ad of availableAds) {
          try {
            ...
            if (tabs.length === oldTabs.length) {
              await this.driver.executeScript("arguments[0].click();", clickable);
              await sleep(4000);
              tabs = await this.driver.getAllWindowHandles();
            }

            if (tabs.length === oldTabs.length) continue;
